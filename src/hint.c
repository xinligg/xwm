...
const char xwmRestart[]       = "_XWM_RESTART";
const char xwmExit[]          = "_XWM_EXIT";
const char xwmReload[]        = "_XWM_RELOAD";
...
static const AtomNode atomList[] = {
   ...
   { &atoms[ATOM_XWM_RESTART],               &xwmRestart[0]                },
   { &atoms[ATOM_XWM_EXIT],                  &xwmExit[0]                   },
   { &atoms[ATOM_XWM_RELOAD],                &xwmReload[0]                 },
   { &atoms[ATOM_XWM_WM_STATE_MAXIMIZED_TOP],
      "_XWM_WM_STATE_MAXIMIZED_TOP" },
   { &atoms[ATOM_XWM_WM_STATE_MAXIMIZED_BOTTOM],
      "_XWM_WM_STATE_MAXIMIZED_BOTTOM" },
   { &atoms[ATOM_XWM_WM_STATE_MAXIMIZED_LEFT],
      "_XWM_WM_STATE_MAXIMIZED_LEFT" },
   { &atoms[ATOM_XWM_WM_STATE_MAXIMIZED_RIGHT],
      "_XWM_WM_STATE_MAXIMIZED_RIGHT" }
...
void WriteNetState(ClientNode *np)
{
   ...
   if(np->state.maxFlags & MAX_TOP) {
      values[index++] = atoms[ATOM_XWM_WM_STATE_MAXIMIZED_TOP];
   }
   if(np->state.maxFlags & MAX_BOTTOM) {
      values[index++] = atoms[ATOM_XWM_WM_STATE_MAXIMIZED_BOTTOM];
   }
   if(np->state.maxFlags & MAX_LEFT) {
      values[index++] = atoms[ATOM_XWM_WM_STATE_MAXIMIZED_LEFT];
   }
   if(np->state.maxFlags & MAX_RIGHT) {
      values[index++] = atoms[ATOM_XWM_WM_STATE_MAXIMIZED_RIGHT];
   }
   ...
   if(status == Success && realFormat != 0) {
      if(count > 0) {
         ...
         for(x = 0; x < count; x++) {
            ...
            } else if(state[x] == atoms[ATOM_XWM_WM_STATE_MAXIMIZED_TOP]) {
               result.maxFlags |= MAX_TOP;
            } else if(state[x] == atoms[ATOM_XWM_WM_STATE_MAXIMIZED_BOTTOM]) {
               result.maxFlags |= MAX_BOTTOM;
            } else if(state[x] == atoms[ATOM_XWM_WM_STATE_MAXIMIZED_LEFT]) {
               result.maxFlags |= MAX_LEFT;
            } else if(state[x] == atoms[ATOM_XWM_WM_STATE_MAXIMIZED_RIGHT]) {
