 ...
 * @author Xin Li
 *
 * @brief The main entry point and related XWM functions.
 *
 */
...
static void SendXWMMessage(const char *message);
...
int main(int argc, char *argv[])
{
   ...
   do {
      ...
      /* Prepare XWM components. */
      Initialize();

      /* Parse the configuration file. */
      ParseConfig(configPath);

      /* Start up the XWM components. */
      Startup();

      /* The main event loop. */
      EventLoop();

      /* Shutdown XWM components. */
      Shutdown();
...
void StartupConnection(void)
{
   ...
   /* Create the supporting window used to verify XWM is running. */
...
/** Startup the various XWM components.
...
/** Shutdown the various XWM components.
...
/** Send _XWM_RESTART to the root window. */
void SendRestart(void)
{
   SendXWMMessage(xwmRestart);
}

/** Send _XWM_EXIT to the root window. */
void SendExit(void)
{
   SendXWMMessage(xwmExit);
}

/** Send _XWM_RELOAD to the root window. */
void SendReload(void)
{
   SendXWMMessage(xwmReload);
}

/** Send a XWM message to the root window. */
void SendXWMMessage(const char *message)
{
   XEvent event;
   OpenConnection();
